...
def task_prompt(number, task_id, work_kind=None):
    ...
    if work_kind == FEATURE_WORK_KIND:
        directions = (
            fallback +
            "Before implementation, create and maintain separate `plan.md` and `tasks.md` files "
            "in the assigned folder. Add discovered required work to `tasks.md` only when it is "
            "required by acceptance, correctness/regression, reuse boundaries, or a demonstrated "
            "quality defect; do not add opportunistic enhancements. Work the next unchecked "
            "non-blocked task; record blockers and continue independent work. Do not close the "
            "feature until every checkbox and acceptance criterion is complete and validated. "
            "Keep shared APIs semantic/config-driven and scene/place/material-ID-specific policy "
            "in composition; prove reuse with an independent consumer/fixture when practical. "
